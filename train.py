...
from src.utils import set_seed, LABELS
from src.trainer import train_and_evaluate
#from src.filtering import filter_dataset_by_similarity_ssim
...
import numpy as np
import torch
from src.trainer import collect_slice_predictions, aggregate_slices
import json
import json
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--train_csv", type=str, default="./results/preprocessed_data/df_test_imgs.csv")
    ...
    parser.add_argument('--model_dir', type=str, default='./models', help='Directory to save models and outputs')
    parser.add_argument('--save_dir', type=str, default='./models', help='Directory to save models and outputs')
    parser.add_argument('--experiment_name', type=str, default='new_model_bbox_giou_brain0.1_l0.1', help='Name of the experiment for WandB')
    parser.add_argument('--n_splits', type=int, default=5, help='Number of cross‑validation splits')
    parser.add_argument('--threshold_brain_presence', type=float, default=0.1, help='threshold for filtering slices')
    parser.add_argument('--dynamic_w', type=str, default="0.25,0.5,1.0", help='threshold for filtering slices') 
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size for training')
    parser.add_argument('--epochs', type=int, default=5000, help='Maximum number of epochs per fold')
    parser.add_argument('--patience', type=int, default=200, help='Number of epochs without improvement before stopping')
    parser.add_argument('--lr', type=float, default=1e-5, help='Learning rate')
    parser.add_argument('--lambda_aux', type=float, default=0.1, help='lambda_aux for centroid of brain')
    
    parser.add_argument('--weight_decay', type=float, default=1e-4, help='Weight decay')
    parser.add_argument('--base_model', type=str, default='maxvit_nano_rw_256.sw_in1k', help='timm model name')
    ...
    parser.add_argument('--no_do_inference', dest='do_inference', action='store_false',
                        help='Disable view information (default: enabled)')
    parser.set_defaults(do_inference=True)
    
    parser.add_argument('--pretrained', type=int, default=0, help='Use pretrained backbone (1=yes, 0=no)')
    parser.add_argument('--image_size', type=int, default=256, help='Image size (square) for resizing slices')
    parser.add_argument('--in_channels', type=int, default=1, help='Number of input channels')
    parser.add_argument('--slice_frac', type=float, default=0.99, help='Fraction of training slices used per epoch')
    ...
    parser.add_argument('--device', type=str, default="cuda", help='CUDA/CPU device (e.g. cuda:0)')
    ...
    parser.add_argument('--norm_mode', type=str, default='dataset_z_per_view', choices=['none', 'slice_z', 'dataset_z_per_view'],
...
def main() -> None:
    ...
    if args.do_inference and args.test_csv:
        ...
        #print(f"df_test Original+patient_id: {test_df.shape}")
        
        print("test_df['filename']:")
        #print(list(test_df['filename']))
        # 📊 Unique patient_id antes del filtrado
        print("[INFO] Antes del filtrado → unique filename:", test_df['filename'].nunique() if 'filename' in test_df else "N/A")
        ...
        test_df = test_df[test_df["ratio"] >= args.threshold_brain_presence].reset_index(drop=True)
        print(f"df_test Filtrado (por ratio): {test_df.shape}")

        # 📊 Unique patient_id después del filtrado
        if 'filename' in list(test_df.columns):
            print("[INFO] Después del filtrado → unique filename:", test_df['filename'].nunique())
        ...
        for fold in range(args.n_splits):
            ...
            if args.norm_mode == 'dataset_z_per_view':
                ...
                stats_file = os.path.join(args.model_dir, f"per_view_stats_fold{fold}.json")
