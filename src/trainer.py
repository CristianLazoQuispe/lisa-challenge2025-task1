...
from utils import (assign_patient_stratified_folds,
                   assign_volume_stratified_folds, compute_sample_weights,
                   compute_weights_from_df)
...
def collect_slice_predictions(
    ...
    model.eval()
    with torch.no_grad():
        for x, y, path, view, aux_target in tqdm(
                loader, desc="Inference", leave=False):
            x = x.to(device, non_blocking=True)
            view = view.to(device, non_blocking=True)
            # if 'view' in model.forward.__code__.co_varnames else model(x)
            y_hat, aux_pred = model(x, view)
...
def compute_stats_per_view(df: pd.DataFrame) -> Dict[str, Dict[str, float]]:
    ...
    for idx in range(len(df)):
        row = df.iloc[idx]
        # or row.get('img_path') or row.get('path') or row.get('filename')
...
def aggregate_slices(results: List[Dict],
                     num_labels: int,
                     num_classes: int,
                     aggregator: str = 'mean',
                     weights: Optional[np.ndarray] = None) -> Tuple[np.ndarray,
                                                                    np.ndarray,
                                                                    Optional[np.ndarray],
                                                                    List[str]]:
    ...
    * ``weighted``: probabilities are combined via a weighted average.
    ...
    for item in results:
        ...
        # Example: path '.../LISA_0001_LF_axi_023.png' ->
    ...
    for i, name in enumerate(vol_names):
        ...
        probs = np.stack([item['probs']
                         for item in group], axis=0)    # (n_slices, L, C)
        preds = np.stack([item['preds']
                         for item in group], axis=0)    # (n_slices, L)
        ...
        if aggregator == 'mean':
            ...
            # still compute mean for completeness
...
def evaluate_aggregators(results: List[Dict],
    ...
    for agg in aggregators:
        ...
        if y_true is None:
            ...
            for i, col in enumerate(label_cols):
                ...
                pred_counts[col] = {cls: int(counts[cls])
        ...
        for i, col in enumerate(label_cols):
            ...
            nll = - \
                np.log(probs_label[np.arange(
                    probs_label.shape[0]), yt] + epsilon)
...
def epoch_subsample_frac_unique_patients(df, frac=0.1, seed=None):
    ...
    selected_patients = rng.choice(
        unique_patients, size=n_select, replace=False)
...
def train_and_evaluate(train_df: pd.DataFrame,
    ...
    # Otherwise, decide how to split: by volume or by patient depending on
    # args.
    if 'fold' in train_df.columns:
        ...
        if split_by_volume:
            ...
            df = assign_volume_stratified_folds(
                train_df,
                volume_id_col=volume_id_col,
            ...
            df = assign_patient_stratified_folds(
                train_df, n_splits=n_splits, seed=args.get('seed', 42))
    ...
    for lbl in label_cols:
        ...
        criterions.append(
            DynamicFocalLoss2(
                alpha=torch.tensor(
                    [
                        float(value) for value in args.get(
                            'dynamic_w',
                            "0.25, 0.75, 1.0").split(",")],
                    dtype=torch.float,
                    device=device),
    ...
    use_wandb = bool(
        os.getenv('WANDB_API_KEY')) and bool(
        os.getenv('PROJECT_WANDB'))
    ...
    for fold in range(n_splits):
        ...
        # Determine normalisation mode and compute per-view stats for this fold
        # if needed
        ...
        if args.get('use_sampling', True):
            ...
            train_loader = DataLoader(
                train_ds, batch_size=args.get(
                    'batch_size', 16), sampler=sampler, num_workers=args.get(
                    'num_workers', 4), pin_memory=True)
        else:
            train_loader = DataLoader(
                train_ds, batch_size=args.get(
                    'batch_size', 16), shuffle=True, num_workers=args.get(
                    'num_workers', 4), pin_memory=True)
        val_loader = DataLoader(
            val_ds, batch_size=args.get(
                'batch_size', 16), shuffle=False, num_workers=args.get(
                'num_workers', 4), pin_memory=True)
        # Instantiate model
        model = Model2DTimm(
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, mode='max', factor=0.95, patience=args.get(
        ...
        best_state_dict = {k: v.detach().cpu()
        ...
        for epoch in range(args.get('epochs', 30)):
            ...
            if args.get('slice_frac', 1.0) < 1.0:
                ...
                train_fold_ep = epoch_subsample_frac_unique_patients(
                    train_fold, frac=0.1, seed=None)

                train_ds_ep = MRIDataset2D(
                    train_fold_ep,
                    is_train=True,
                    use_augmentation=True,
                    is_numpy=True,
                ...
                if args.get('use_sampling', True):
                    sample_weights = compute_sample_weights(
                        train_fold_ep, label_cols)
                    sampler = WeightedRandomSampler(
                        sample_weights, len(sample_weights), replacement=True)
                    train_loader = DataLoader(
                        train_ds_ep, batch_size=args.get(
                            'batch_size', 16), sampler=sampler, num_workers=args.get(
                            'num_workers', 4), pin_memory=True)
                else:
                    train_loader = DataLoader(
            ...
            for step, (x, y, _, view, aux_target) in enumerate(pbar, start=1):
                ...
                view = view.to(device, non_blocking=True)
                aux_target = aux_target.to(
                    device, non_blocking=True)  # (B,3) en [0,1]

                optimizer.zero_grad(set_to_none=True)
                with torch.cuda.amp.autocast():
                    y_hat, aux_pred = model(
                        x, view) if args.get(
                        'use_view', False) else model(x)
                    cls_loss = sum(criterions[i](y_hat[:, i], y[:, i]) for i in range(
                        len(label_cols))) / len(label_cols)

                    # aux_loss = F.smooth_l1_loss(aux_pred, aux_target)
                    # loss = cls_loss + args.get("lambda_aux", 0.2) * aux_loss
                    aux_l1 = torch.nn.functional.smooth_l1_loss(
                        aux_pred, aux_target)
                    aux_giou = (1.0 - giou_yolo(aux_pred, aux_target)).mean()
                    aux_loss = 0.5 * aux_l1 + 0.5 * aux_giou
                    # súbelo a 0.3–1.0 para probar
                    loss = cls_loss + args.get("lambda_aux", 0.5) * aux_loss
                ...
                nbatches += 1
                # Do not log per batch.  Logging happens at the end of the
                # epoch with a global step.
            epoch_loss = running_loss / max(1, nbatches)
            # ------- Validation at epoch end -------
            # 1) Compute slice‑level validation loss to compare with training
            # loss.
            model.eval()
            val_loss_total = 0.0
            n_val_batches = 0
            with torch.no_grad():
                for x_val, y_val, _, view_val, aux_target_val in val_loader:
                    ...
                    aux_target_val = aux_target_val.to(
                        device, non_blocking=True)  # (B,3) en [0,1]
                    with torch.cuda.amp.autocast():
                        y_hat_val, aux_pred_val = model(
                            x_val, view_val) if args.get(
                            'use_view', False) else model(x_val)
                        cls_loss_val = sum(
                        ...
                        aux_l1_val = torch.nn.functional.smooth_l1_loss(
                            aux_pred_val, aux_target_val)
                        aux_giou_val = (
                            1.0 -
                            giou_yolo(
                                aux_pred_val,
                        ...
                        # súbelo a 0.3–1.0 para probar
                        loss_val = cls_loss_val + \
                            args.get("lambda_aux", 0.5) * aux_loss_val

                    val_loss_total += float(loss_val.item())
                    n_val_batches += 1
            val_loss_slice_level = val_loss_total / max(1, n_val_batches)
            # 2) Collect per‑slice predictions on val set for aggregator
            # evaluation
            val_results = collect_slice_predictions(val_loader, model, device)
            val_metrics = evaluate_aggregators(
                val_results, label_cols, num_classes=3, aggregators=aggregators)
            ...
            # ------- Logging per epoch -------
            # Print a summary of validation metrics for each aggregator to the
            # console
            print(
                f"Fold {fold} Epoch {epoch} (train_loss={
                    epoch_loss:.4f}, val_loss={
                    val_loss_slice_level:.4f}):",
                end=" ")
            summary_strs = []
            ...
            # Accumulate metrics into a dictionary and log once per epoch using
            # local step
            if use_wandb and run is not None:
                ...
                for agg_name, m in val_metrics.items():
                    if 'f1_macro' in m:
                        log_data[f'val_global/val_{
                            agg_name}_f1_macro'] = m['f1_macro']
                        log_data[f'val_global/val_{
                            agg_name}_f1_micro'] = m['f1_micro']
                        log_data[f'val_global/val_{agg_name}_acc'] = m['acc']
                        log_data[f'val_global/val_{agg_name}_loss_macro'] = m.get(
                            'loss_macro', 0.0)
                        for lbl in label_cols:
                            log_data[f'val_{
                                agg_name}_f1/{lbl}'] = m['per_label_f1'].get(lbl, 0.0)
                            log_data[f'val_{
                                agg_name}_loss/{lbl}'] = m['per_label_loss'].get(lbl, 0.0)
                            for cls in range(3):
                                cnt = m['pred_counts'][lbl].get(cls, 0)
                                log_data[f'val_{
                                    agg_name}_counts/{lbl}_{cls}'] = cnt
        ...
        best_val_metrics = evaluate_aggregators(
            best_val_results, label_cols, num_classes=3, aggregators=aggregators)
        ...
        if test_back_df is not None and not test_back_df.empty:
            test_back_ds = MRIDataset2D(
                ...
                norm_mode=norm_mode,
                per_view_stats=per_view_stats)
            test_back_loader = DataLoader(
                test_back_ds, batch_size=args.get(
                    'batch_size', 16), shuffle=False, num_workers=args.get(
                    'num_workers', 4), pin_memory=True)
            test_back_results = collect_slice_predictions(
                test_back_loader, model, device)
            test_back_metrics = evaluate_aggregators(
                test_back_results, label_cols, num_classes=3, aggregators=aggregators)
        # Print final validation metrics and optional test_back metrics for
        # this fold
        ...
        def _save_preds(metrics: Dict[str, Dict], dataset_name: str) -> None:
            for agg_name, m in metrics.items():
                # y_prob and y_pred are only present when y_true is available
                # (val/test_back)
                if 'y_prob' in m and 'y_pred' in m and 'names' in m:
                    ...
                    for i, lbl in enumerate(label_cols):
                        ...
                        for cls_idx in range(y_prob_arr.shape[-1]):
                            df_pred[f'{lbl}_{
                                cls_idx}'] = y_prob_arr[:, i, cls_idx]
                    # Ensure directory exists
                    agg_dir = os.path.join(out_dir, f'preds_fold{fold}')
                    os.makedirs(agg_dir, exist_ok=True)
                    # File path
                    file_name = f'{dataset_name}_{
                        agg_name}_fold{fold}_probs.csv'
                    file_path = os.path.join(agg_dir, file_name)
                    df_pred.to_csv(file_path, index=False)
                    print(f"Saved {dataset_name} predictions for aggregator '{
                          agg_name}' to {file_path}")

                    file_name = f'{dataset_name}_{
                        agg_name}_fold{fold}_preds.csv'
                    file_path = os.path.join(agg_dir, file_name)
                    df_pred[["filename"] +
                            label_cols].to_csv(file_path, index=False)
                    print(f"Saved {dataset_name} predictions for aggregator '{
                          agg_name}' to {file_path}")
        ...
        if use_wandb and run is not None:
            # Prepare summary metrics for this fold (best F1 and optional
            # test_back)
            ...
            if test_back_metrics:
                for agg_name, m in test_back_metrics.items():
                    if 'f1_macro' in m:
                        summary_data[f'test_back_global/test_back_{
                            agg_name}_f1_macro'] = m['f1_macro']
                        summary_data[f'test_back_global/test_back_{
                            agg_name}_f1_micro'] = m['f1_micro']
                        summary_data[f'test_back_global/test_back_{
                            agg_name}_acc'] = m['acc']
                        summary_data[f'test_back_global/test_back_{
                            agg_name}_loss_macro'] = m.get('loss_macro', 0.0)
                        for lbl in label_cols:
                            summary_data[f'test_back_{
                                agg_name}_f1/{lbl}'] = m['per_label_f1'][lbl]
                            summary_data[f'test_back_{
                                agg_name}_loss/{lbl}'] = m['per_label_loss'][lbl]
                            for cls in range(3):
                                cnt = m['pred_counts'][lbl].get(cls, 0)
                                summary_data[f'test_back_{
                                    agg_name}_counts/{lbl}_{cls}'] = cnt
    ...
    # For each aggregator, average probabilities across folds and recompute
    # predictions
    ensemble_results = {}
    if n_splits > 1:
        ...
        for fr in fold_results:
            ...
            if test_back_df is not None and fr['test_back_metrics']:
                for agg in aggregators:
                    ...
                    test_names = m['names']
        # Compute ensemble metrics by aggregating metrics across folds rather
        # than stacking
        val_ensemble_metrics: Dict[str, Dict] = {}
        test_ensemble_metrics: Dict[str, Dict] = {}
        for agg in aggregators:
            # Validation ensemble: average metrics across folds
            metrics_list = [fr['val_metrics'][agg]
                            for fr in fold_results if agg in fr['val_metrics']]
            if metrics_list:
                f1_macro_avg = float(
                    np.mean([m['f1_macro'] for m in metrics_list]))
                f1_micro_avg = float(
                    np.mean([m['f1_micro'] for m in metrics_list]))
                acc_avg = float(np.mean([m['acc'] for m in metrics_list]))
                per_label_f1_avg: Dict[str, float] = {}
                per_label_loss_avg: Dict[str, float] = {}
                pred_counts_sum: Dict[str, Dict[int, int]] = {
                    lbl: {0: 0, 1: 0, 2: 0} for lbl in label_cols}
                for lbl in label_cols:
                    per_label_f1_avg[lbl] = float(
                        np.mean([m['per_label_f1'][lbl] for m in metrics_list]))
                    per_label_loss_avg[lbl] = float(
                        np.mean([m['per_label_loss'][lbl] for m in metrics_list]))
                    # Sum counts across folds
                    for m in metrics_list:
                        for cls in range(3):
                            pred_counts_sum[lbl][cls] += m['pred_counts'][lbl].get(
                                cls, 0)
                loss_macro_avg = float(
                    np.mean(list(per_label_loss_avg.values())))
            ...
            if test_back_df is not None:
                ...
                if metrics_test_list:
                    f1_macro_avg = float(
                        np.mean([m['f1_macro'] for m in metrics_test_list]))
                    f1_micro_avg = float(
                        np.mean([m['f1_micro'] for m in metrics_test_list]))
                    acc_avg = float(np.mean([m['acc']
                    ...
                    for lbl in label_cols:
                        per_label_f1_avg[lbl] = float(
                            np.mean([m['per_label_f1'][lbl] for m in metrics_test_list]))
                        per_label_loss_avg[lbl] = float(
                            np.mean([m['per_label_loss'][lbl] for m in metrics_test_list]))
                        for m in metrics_test_list:
                            for cls in range(3):
                                pred_counts_sum[lbl][cls] += m['pred_counts'][lbl].get(
                                    cls, 0)
        ...
        if use_wandb and (val_ensemble_metrics or test_ensemble_metrics):
            ...
            for agg_name, m in val_ensemble_metrics.items():
                ...
                log_data[f'ensemble_val_{agg_name}_loss_macro'] = m.get(
                    'loss_macro', 0.0)
                for lbl in label_cols:
                    log_data[f'ensemble_val_{
                        agg_name}_f1/{lbl}'] = m['per_label_f1'][lbl]
                    log_data[f'ensemble_val_{
                        agg_name}_loss/{lbl}'] = m['per_label_loss'][lbl]
                    for cls in range(3):
                        log_data[f'ensemble_val_{
                            agg_name}_counts/{lbl}_{cls}'] = m['pred_counts'][lbl][cls]
            for agg_name, m in test_ensemble_metrics.items():
                log_data[f'ensemble_test_back_global/ensemble_test_back_{
                    agg_name}_f1_macro'] = m['f1_macro']
                log_data[f'ensemble_test_back_global/ensemble_test_back_{
                    agg_name}_f1_micro'] = m['f1_micro']
                log_data[f'ensemble_test_back_global/ensemble_test_back_{
                ...
                log_data[f'ensemble_test_back_global/ensemble_test_back_{
                    agg_name}_loss_macro'] = m.get('loss_macro', 0.0)
                for lbl in label_cols:
                    log_data[f'ensemble_test_back_{
                        agg_name}_f1/{lbl}'] = m['per_label_f1'][lbl]
                    log_data[f'ensemble_test_back_{
                        agg_name}_loss/{lbl}'] = m['per_label_loss'][lbl]
                    for cls in range(3):
