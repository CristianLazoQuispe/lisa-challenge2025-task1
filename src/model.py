...
class LabelTokenHead(nn.Module):
    ...
    def forward(self, feats: torch.Tensor) -> torch.Tensor:
        ...
        tokens = self.label_tokens.unsqueeze(0).expand(
            B, -1, -1).contiguous()  # (B, L, d)
...
class RegHead(nn.Module):
    def __init__(self, feat_dim):
        ...
        with torch.no_grad():
            ...
            self.out.bias[:] = torch.tensor(
                [0.0, 0.0, -0.62, -0.62])  # sigmoid(-0.62)~0.35
