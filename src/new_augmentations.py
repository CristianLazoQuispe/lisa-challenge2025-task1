...
def _brain_margins_connected(
    x2d: np.ndarray,
    # % mínimo del área de la imagen para aceptar una componente
    min_area_frac: float = 0.02,
    blur_ks: int = 3,              # suavizado previo para Otsu
    close_ks: int = 5,             # cierre morfológico para rellenar huecos
    # limpia rayas finas (zipper) antes de los CC
    stripe_clean: bool = True
):
    """
    ...
    if mask.mean() < 1.0:  # demasiado vacío → fallback percentil
        ...
        _, mask = cv2.threshold(
            (x * 255).astype(np.uint8), thr, 255, cv2.THRESH_BINARY)

    # 3) Limpieza morfológica
    if stripe_clean:
        # abre con kernels lineales para quitar rayas finas
...
class RandomZipperStripe(A.ImageOnlyTransform):
    ...
    def apply(self, img, **params):
        ...
        # Elegir eje
        axis = self.axis if self.axis != "rand" else (
            "x" if np.random.rand() < 0.5 else "y")
        ...
        out = x + amp * stripe
        # Asegura contigüidad/writable para siguientes pasos (ToTensorV2/own
        # tensor):
...
class RandomBandCut(A.ImageOnlyTransform):
    ...
    def apply(self, img, **params):
        ...
        thick = int(max(1, (h if is_h else w) *
                        np.random.uniform(0.01, self.max_thick_ratio)))
        if is_h:
